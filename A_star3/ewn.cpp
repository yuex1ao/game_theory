...
int EWN::heuristic(int move, int plies) { // cost function
    ...
    // 如果沒有指定goal_piece的話是否可以利用dice_seq以及distance來對每個dice進行判斷。
...
int EWN::heuristic2(int plies) {
    ...
    if(!piece) { // 沒有指定piece的話就先跑一輪決定goal_piece
        int goal = 0, minCost = INT_MAX, minTemp = 0;
        for(int i = 1; i <= 6; i++) {
            if(pos[i] == -1) continue;
            goal_piece = i;
            minTemp = heuristic2(plies);
            if(minTemp < minCost) {
                goal = i;
                minCost = minTemp;
            }
    ...
    int dr = pos[piece] / COL + 1, dc = pos[piece] % COL + 1;
    int distance_goal = max(ROW - dr, COL - dc);
    // 幹掉小於goal_piece的dice。
    for(int i = piece - 1; i >= 1; i--) {
        ...
        int distance = distance_goal, small = i, large = piece;
        while(pos[small] == -1) small--;
        while(pos[large] == -1) large++; 
    ...
    for(int i = piece + 1; i <= 6; i++) { // 先估計哪些子必須、值得幹掉。
        ...
        int distance = distance_goal, small = piece, large = i;
        while(pos[small] == -1) small--;
        while(pos[large] == -1) large++; 
        while(distance > 0 && cost2[i] < period * ROW) {
            if(dice_seq[seq] >= small && dice_seq[seq] <= large) {
...
void EWN::A_Star(Node *ancestor, bool &find) {
    ...
    for(int i = 0; i < n_move; i++) {
        ...
        int cost = heuristic2(ancestor->cost_g + 1);
        Node *node = new Node();
