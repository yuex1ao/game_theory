...
int EWN::heuristic(int plies) {
    ...
    if(freq[piece]) {
        int seq = plies % period;
        int distance = distance_goal, small = piece, large = piece;
        while(pos[small - 1] == -1) small--;
        while(pos[large + 1] == -1) large++;
...
void EWN::A_Star(shared_ptr<Node> &ancestor, bool &find) {
    if(!goal_piece) {
        setGoal();
    }
    int moves[MAX_MOVES];
